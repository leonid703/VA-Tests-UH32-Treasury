...
Сценарий: 05.2. Оплата платежных поручений
	...
	* Оплата поставщику, 0000-000001
		...
		И в таблице "Список" я перехожу к строке:
			| 'Вид операции (БП)' | 'Организация'             | 'Получатель'  | 'Состояние'    | 'Сумма'    |
			| 'Оплата поставщику' | 'Королевство Гор и Долин' | 'Контрагент1' | 'Подготовлено' | '1 000,00' |
	...
	* Уплата налога, 0000-000003
		...
		И в таблице "Список" я перехожу к строке:
			| 'Вид операции (БП)' | 'Организация'             | 'Получатель'          | 'Состояние'    | 'Сумма'  |
			| 'Уплата налога'     | 'Королевство Гор и Долин' | 'Налоговая инспекция' | 'Подготовлено' | '400,00' |
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Платежное поручение * от *'
	...
	* Прочие расчеты с контрагентами, 0000-000006 
		...
		И в таблице "Список" я перехожу к строке:
			| 'Вид операции (БП)'              | 'Организация'             | 'Получатель'  | 'Состояние'    | 'Сумма'      |
			| 'Прочие расчеты с контрагентами' | 'Королевство Гор и Долин' | 'Контрагент1' | 'Подготовлено' | '100 000,00' |
		И в таблице "Список" я выбираю текущую строку
		Тогда открылось окно 'Платежное поручение * от *'
